...
//
// 模块化说明：
// 这里属于 YOLO/RKNN 检测模块内部的日志控制，只影响 detection 侧的逐帧
// 调试输出，不影响摄像头采集、预览窗口、视频保存或大模型推理。
//
// 默认关闭的原因：
// - camera 实时检测会每帧调用一次 inference_yolov8_model()；
// - 如果每帧都 printf("rknn_run")，终端会刷屏，掩盖真正的错误日志；
// - 高频 printf 也会给实时循环增加不必要的 IO 开销。
//
// 需要确认每帧是否真的进入 NPU 推理时，可以在板端设置：
...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    // 执行 YOLO RKNN 推理。
    //
    // 注意：这里位于实时检测的逐帧路径中。默认不打印 rknn_run，避免 SSH/串口
    // 终端被刷屏；需要逐帧调试时再通过 DEMO_VERBOSE_RKNN=1 打开。
