...
static std::string select_existing_path(const char* env_name, const char* fallback, const char* alt_path)
{
    ...
    if (alt_path && access(alt_path, R_OK) == 0) {
...
float* process_image(const char* image_path, rknn_app_context_t* rknn_app_ctx, size_t& n_image_tokens) {
    ...
    // 🔧 修复1: 确保内存连续（避免 .data 指针失效）
    if (!resized_img.isContinuous()) {
        resized_img = resized_img.clone();
    }
    
    size_t n_tokens = IMAGE_TOKEN_NUM;      // 196
    size_t embed_size = IMAGE_EMBED_SIZE;     // 1536 for Qwen2-VL-2B
    size_t embed_len = n_tokens * embed_size;
    ...
    memset(img_vec, 0, embed_len * sizeof(float));  // 初始化为0，避免垃圾值
    
    // 🔧 调试打印：确认参数正确
    printf("[DEBUG] process_image:\n");
    printf("  model_size: %dx%d, channel: %d\n", 
           rknn_app_ctx->model_width, 
           rknn_app_ctx->model_height,
           rknn_app_ctx->model_channel);
    printf("  n_tokens: %zu, embed_size: %zu\n", 
           n_tokens, embed_size);
    printf("  img_vec allocated: %zu floats (%.2f MB)\n", 
           embed_len, embed_len * sizeof(float) / 1024.0 / 1024.0);
    ...
    // 🔧 调试打印：验证输出数据有效性
    printf("[DEBUG] img_vec sample values:\n");
    printf("  [0]=%8.4f, [100]=%8.4f, [1000]=%8.4f, [%zu]=%8.4f\n",
           img_vec[0], 
           img_vec[100], 
           img_vec[1000],
           embed_len - 1, img_vec[embed_len - 1]);
    
    // 检查是否全为零（编码失败的特征）
    bool all_zero = true;
    for (size_t i = 0; i < std::min(embed_len, size_t(1000)); i++) {
        if (fabs(img_vec[i]) > 1e-6) {
            all_zero = false;
            break;
        }
    }
    if (all_zero) {
        cerr << "警告: img_vec 全为零，可能编码失败!" << endl;
    }
    
    // 🔧 修复3: 通过引用返回正确的 token 数 (196)，不是 embed_size (2048)
    n_image_tokens = n_tokens;  // ✅ 196
    
    printf("[DEBUG] process_image success, n_image_tokens=%zu\n", n_image_tokens);
    return img_vec;
}
...
int main()
{
    ...
    const std::string yolo_model_path = select_existing_path(
        "DEMO_YOLO_MODEL",
        "ztl_yolov8n_rk3588_i8_ver01.rknn",
        "model/ztl_yolov8n_rk3588_i8_ver01.rknn");
    const std::string image_path = select_existing_path(
        "DEMO_IMAGE",
        "model/bus.jpg",
        "bus.jpg");
    const std::string encoder_model_path = select_existing_path(
        "DEMO_VISION_MODEL",
        "Qwen2-VL-2B_vision_rk3588.rknn",
        "model/Qwen2-VL-2B_vision_rk3588.rknn");
    const std::string llm_model_path = select_existing_path(
        "DEMO_LLM_MODEL",
        "qwen2_vl_2b_instruct.rkllm",
        "model/qwen2_vl_2b_instruct.rkllm");
    const std::string upload_host = getenv_string_or("DEMO_HOST", "192.168.123.198:8000");
    const bool upload_enabled = getenv_bool_or("DEMO_UPLOAD", true);
    int max_new_tokens = getenv_int_or("DEMO_MAX_NEW_TOKENS", 128);
    int max_context_len = getenv_int_or("DEMO_MAX_CONTEXT_LEN", 512);
    int rknn_core_num = getenv_int_or("DEMO_RKNN_CORE_NUM", 3);

    printf("[CONFIG] DEMO_YOLO_MODEL=%s\n", yolo_model_path.c_str());
    printf("[CONFIG] DEMO_IMAGE=%s\n", image_path.c_str());
    printf("[CONFIG] DEMO_VISION_MODEL=%s\n", encoder_model_path.c_str());
    printf("[CONFIG] DEMO_LLM_MODEL=%s\n", llm_model_path.c_str());
    printf("[CONFIG] DEMO_HOST=%s, DEMO_UPLOAD=%s\n",
           upload_host.c_str(), upload_enabled ? "on" : "off");

    // 初始化权重
    yolov8_ret = init_yolo_weights(yolo_model_path.c_str(), &app_ctx);
    if (yolov8_ret != 0) {
        return yolov8_ret;
    }
    ...
    param.model_path = llm_model_path.c_str();
    param.top_k = 1;
    ...
    ret = init_imgenc(encoder_model_path.c_str(), &rknn_app_ctx, core_num);
    if (ret != 0) {
        printf("init_imgenc fail! ret=%d model_path=%s\n", ret, encoder_model_path.c_str());
        release_yolo_resources(&app_ctx);
    ...
    float* img_vec = process_image(image_path.c_str(), &rknn_app_ctx, n_image_tokens);
    if (!img_vec) {
        cerr << "初始图像处理失败!" << endl;
        release_yolo_resources(&app_ctx);
    ...
    while(true) {
        ...
        if (input_str == "clear")
        {
            ...
            printf("当前 RKLLM 头文件未暴露 clear kv cache 接口，已跳过。\n");
            continue;
        }
        ...
        if (input_str == "camera_shot") {
            ...
            string raw_prompt = "<image>客观分析图中情况。";
            string prompt = string(PROMPT_TEXT_PREFIX) + raw_prompt + PROMPT_TEXT_POSTFIX;
            cout << "自动提问: " << raw_prompt << endl;
            ...
            // RKLLM API compatibility: rkllm_input.multimodal_input.image_height = rknn_app_ctx.model_height;
            // RKLLM API compatibility: rkllm_input.multimodal_input.image_width = rknn_app_ctx.model_width;
            // 执行推理
            printf("robot: ");
            size_t result_len = 0;
        ...
        if (input_str.substr(0, 5) == "load ") {
            ...
            if (!img_vec) {
                ...
                string raw_prompt = "<image>客观分析图中情况。";
                input_str = string(PROMPT_TEXT_PREFIX) + raw_prompt + PROMPT_TEXT_POSTFIX;
                cout << "自动提问: " << raw_prompt << endl;
        ...
        for (int i = 0; i < (int)pre_input.size(); i++)
        {
            ...
            {
                string raw_prompt = pre_input[i];
                input_str = string(PROMPT_TEXT_PREFIX) + raw_prompt + PROMPT_TEXT_POSTFIX;
                cout << raw_prompt << endl;
            }
        }
        ...
        if (input_str.find("<image>") == std::string::npos) 
        {
            ...
            rkllm_input.input_type = RKLLM_INPUT_MULTIMODAL;
            // RKLLM API compatibility: rkllm_input.role = "user";
            if (input_str.find("<|im_start|>") == std::string::npos) {
                formatted_prompt = string(PROMPT_TEXT_PREFIX) + input_str + PROMPT_TEXT_POSTFIX;
                rkllm_input.multimodal_input.prompt = (char*)formatted_prompt.c_str();
            } else {
                rkllm_input.multimodal_input.prompt = (char*)input_str.c_str();
            }
            rkllm_input.multimodal_input.image_embed = img_vec;
