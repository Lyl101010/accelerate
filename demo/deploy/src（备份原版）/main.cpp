...
#include "common/system_result.h"
#include "border_kg.h"
...
std::string build_chat_prompt(const std::string& prompt) {
    if (prompt.find("<|im_start|>") != std::string::npos) {
        return prompt;
    }
    return std::string(PROMPT_TEXT_PREFIX) + prompt + PROMPT_TEXT_POSTFIX;
}
...
static string upload_host() {
    const char* value = getenv("DEMO_HOST");
    return value && value[0] != '\0' ? value : "192.168.0.100:8000";
}

static string normalized_env_value(const char* value) {
...
static bool upload_enabled() {
    const char* mode_value = getenv("DEMO_UPLOAD_MODE");
    if (mode_value && mode_value[0] != '\0') {
        const string mode = normalized_env_value(mode_value);
        if (mode == "online") {
            return true;
        }
        if (mode == "offline") {
        ...
        cerr << "[Warning] Invalid DEMO_UPLOAD_MODE=" << mode_value
             << ", using offline mode." << endl;
        return false;
    }

    // Keep the previous boolean switch working, but default to local-only inference.
    const char* value = getenv("DEMO_UPLOAD");
    if (!value || value[0] == '\0') {
        return false;
    }
    const string setting = normalized_env_value(value);
    return setting != "0" && setting != "false" && setting != "off" && setting != "no";
}

static int positive_env_int_or(const char* name, int fallback, int max_allowed) {
    ...
    if (!end || *end != '\0' || parsed <= 0 || parsed > max_allowed) {
        cerr << "[Warning] Invalid " << name << "=" << value
             << ", using " << fallback << "." << endl;
        return fallback;
    }
...
int main()
{
    ...
    const string result_upload_host = upload_host();
    const bool result_upload_enabled = upload_enabled();
    cout << "[CONFIG] Result delivery: "
         << (result_upload_enabled ? "online -> " + result_upload_host
    ...
    int max_new_tokens = positive_env_int_or("DEMO_MAX_NEW_TOKENS", 128, 4096);
    int max_context_len = positive_env_int_or("DEMO_MAX_CONTEXT_LEN", 512, 32768);
    int rknn_core_num = positive_env_int_or("DEMO_RKNN_CORE_NUM", 3, 3);
    ...
    while(true) {
        ...
        if (input_str.find("<image>") == std::string::npos) 
        {
            ...
            // 注意：如果是手动输入的 <image> 指令，这里没有经过 YOLO+KG 流程
            // 如果需要在这里也加入 KG，需要重新设计交互逻辑，目前保持原样
            rkllm_input.input_type = RKLLM_INPUT_MULTIMODAL;
            rkllm_input.multimodal_input.prompt = (char*)model_prompt.c_str();
            rkllm_input.multimodal_input.image_embed = img_vec;
            rkllm_input.multimodal_input.n_image_tokens = n_image_tokens;
